...
radial_init()
{
	...
	foreach ( button_group in level._radial_button_group )
	{
...
debug_toggle()
{
	...
	while ( 1 )
	{
		...
		button_reset = true;
		while ( !( level._observer buttonPressed( "BUTTON_Y" ) ) )
			wait 0.05;
		
		level._observer playsound("mouse_click");
...
readyPlayer()
{
	...
	classes = getArrayKeys( level._classMap );
	okclasses = [];
...
get_right_stick_angle()
{
	...
	while ( 1 )
	{
		...
		level._rs_angle = int( rs_angles[1] );			
...
{
	if ( isdefined( level._radial_button_group ) && level._radial_button_group.size )
		assertex( !isdefined( level._radial_button_group[ group_name ] ), "Radial button group: " + group_name + " is already defined." );
	...
	level._radial_button_group[ group_name ] = [];
	level._radial_button_group_info[ group_name ][ "view_start" ] 		= view_start;
	level._radial_button_group_info[ group_name ][ "view_pos" ] 			= player_view_ent.origin + extruded_vec;
	level._radial_button_group_info[ group_name ][ "player_view_pos" ] 	= player_view_ent.origin;
	level._radial_button_group_info[ group_name ][ "view_angles" ] 		= player_view_ent.angles;
}


newRadialButton( button_group, button_label, button_ent_name, action_func )
{
	assertex( isdefined( level._radial_button_group[ button_group ] ), "Radial button group: " + button_group + " does not exist." );
	...
	level._radial_button_group[ button_group ][ level._radial_button_group[ button_group ].size ] = button;
	return button;
}
...
updateSelectedButton()
{
	...
	while ( 1 )
	{
		if ( !isdefined( level._radial_button_current_group ) )
		{
			wait 0.05;
			continue;
		}
		
		last_active_button = level._active_button;
		
		foreach ( button in level._radial_button_group[ level._radial_button_current_group ] )
		{
			if ( isInRange( button.start_angle, button.end_angle ) )	
				level._active_button = button;
			else
				button.font_color	= ( 0.5, 0.5, 1 );
		}
		
		if ( isdefined ( level._active_button ) )
		{
			level._active_button.font_color	= ( 1, 1, 0.5 );
		
			if ( isdefined( last_active_button ) && last_active_button != level._active_button )
				level._observer playsound("mouse_over");	
		}
...
watchSelectButtonPress()
{
	...
	while ( 1 )
	{
		...
		button_reset = true;		
		while ( !( level._observer buttonPressed( "BUTTON_A" ) ) )
			wait 0.05;
			
		level._observer playsound("mouse_click");
				
		if ( isdefined( level._active_button ) && button_reset )
		{
			level._active_button notify( "select_button_pressed" );
			[[level._active_button.action_func]]();
			button_reset = false;
		}
...
watchBackButtonPress()
{
	...
	while ( 1 )
	{
		...
		button_reset = true;
		while ( !( level._observer buttonPressed( "BUTTON_X" ) ) )
			wait 0.05;
...
{
	foreach ( button in level._radial_button_group[ button_group ] )
...
draw_radial_button( button_group )
{
	...
	floating_origin = level._radial_button_group_info[ button_group ][ "view_pos" ];
	...
	while ( 1 )
	{
		...
		if ( isdefined( level._crib_debug ) && level._crib_debug > 0 )
		{
			...
			r_radial_pos = floating_origin + radial_angle_to_vector( level._rs_angle, 2 );
...
{
	level._active_button = undefined;
	
	assertex( isdefined( level._observer ), "Missing observer (connected player), can not attach player to view path" );
	
	if ( isdefined( level._radial_button_current_group ) && level._radial_button_current_group != "" )
	{
		level._radial_button_previous_group = level._radial_button_current_group;
	}
	else
	{
		level._radial_button_previous_group = "main";
		level._radial_button_current_group = "main";
	}
	
	foreach ( button in level._radial_button_group[ level._radial_button_previous_group ] )
			button notify( "remove_button" );

	//iPrintLnBold( "flying to: " + button_group );
	
	if ( isdefined( reverse ) && reverse )
		level._observer go_path_by_targetname_reverse( level._radial_button_group_info[ level._radial_button_previous_group ][ "view_start" ], button_group );
	else
		level._observer go_path_by_targetname( level._radial_button_group_info[ button_group ][ "view_start" ] );
	
	level thread draw_radial_buttons( button_group );
	level._radial_button_current_group = button_group;
}
...
{
	assertex( isdefined( level._radial_button_group[ button_group ] ), "getRadialAngleFromEnt: Radial button group does not exist." );
	assertex( isdefined( ent ), "getRadialAngleFromEnt: Missing entity to be measured." );
	
	rAngle 		 = level._radial_button_group_info[ button_group ][ "view_angles" ];
	rPos 		 = level._radial_button_group_info[ button_group ][ "view_pos" ];
...
{
	inside_big_angle 	= ( level._rs_angle > start_angle && level._rs_angle < 360 );
	inside_small_angle 	= ( level._rs_angle > 0 && level._rs_angle < end_angle );
	
	if ( start_angle > end_angle )
		in_range = ( inside_big_angle || inside_small_angle );
	else
		in_range = ( level._rs_angle > start_angle && level._rs_angle < end_angle );
...
action_back()
{
	...
	/*else*/ if ( isdefined( level._radial_button_current_group ) && level._radial_button_current_group != "main" )
...
{
	level._view_paths[ path_name ] = [];
	
	path_node = getent( path_name, "targetname" );
	level._view_paths[ path_name ][ level._view_paths[ path_name ].size ] = path_node;
	
	while( isdefined( path_node ) && isdefined( path_node.target ) )
	{
		next_node = getent( path_node.target, "targetname" );
		level._view_paths[ path_name ][ level._view_paths[ path_name ].size ] = next_node;
		path_node = next_node;
	}
}
...
{	
	// self is player
	if ( !isdefined( level._dummy_mover ) )
	{
		start_node = level._view_paths[ path_name ][ 0 ];
		level._dummy_mover = spawn( "script_model", start_node.origin );
		level._dummy_mover.angles = start_node.angles;
		//self AllowedStances( "stand" );
		self setOrigin( level._dummy_mover.origin - ( 0, 0, 65 ) );
		self linkTo( level._dummy_mover );
		wait 0.05;
		self setplayerangles ( level._dummy_mover.angles );
	...
	total_distance = abs( distance( level._dummy_mover.origin, level._view_paths[ path_name ][ level._view_paths[ path_name ].size - 1 ].origin ) );
	...
	if ( !CONST_direct_travel )
		blur_time *= travel_speed * ( level._view_paths[ path_name ].size + 1 );
	self thread blur_sine( CONST_blur_strength, blur_time );
	
	foreach ( idx, node in level._view_paths[ path_name ] )
	{
		...
		if ( CONST_direct_travel )
		{
			if ( idx != level._view_paths[ path_name ].size - 1 )
				continue;
		}
			
		//level.dummy_mover MoveTo( node.origin, travel_speed );
		level._dummy_mover MoveTo( node.origin, travel_speed, travel_speed * 0.5, 0 );
		level._dummy_mover RotateTo( node.angles, travel_speed, travel_speed * 0.5, 0);
		wait travel_speed;
	}
}
...
{	
	assertex( isdefined( level._dummy_mover ), "go_path_by_targetname_reverse called before go_path_by_targetname" );
	
	travel_speed = CONST_view_travel_unit_time;
	total_distance = abs( distance( level._dummy_mover.origin, level._radial_button_group_info[ back_to_button_group ][ "player_view_pos" ] ) );
	travel_speed *= total_distance / CONST_view_travel_unit_dist;
	travel_speed = max( travel_speed, 0.1 );	// due to repeated button presses, the travel distance can be cut to 0 travel speed at times.
	...
	if ( !CONST_direct_travel )
	{
		for ( idx = level._view_paths[ path_name ].size - 1; idx >= 0; idx-- )
		{
			node = level._view_paths[ path_name ][ idx ];
			level._dummy_mover MoveTo( node.origin, travel_speed );
			level._dummy_mover RotateTo( node.angles, travel_speed );
	...
	pos = level._radial_button_group_info[ back_to_button_group ][ "player_view_pos" ];
	angle = level._radial_button_group_info[ back_to_button_group ][ "view_angles" ];
	
	level._dummy_mover MoveTo( pos, travel_speed, travel_speed * 0.5, 0 );
	level._dummy_mover RotateTo( angle, travel_speed, travel_speed * 0.5, 0 );
	wait travel_speed;
}
...
force_player_angles()
{
	level endon( "game_ended" );
	self endon( "disconnect" );
	level._dummy_mover endon( "remove_dummy" );
