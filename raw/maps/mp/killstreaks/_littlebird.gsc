...
spawnAttackLittleBird( owner, pathStart, pathGoal, coord )
{
	...
	level._littlebird[level._littlebird.size] = lb;
...
selectLbStrikeLocation( lifeId )
{
	...
	self beginLocationSelection( "map_artillery_selector", true, false, 500 );
...
lbExplode()
{
	...
	playfx ( level._chopper_fx["explode"]["air_death"], self.origin, forward );

	deathAngles = self getTagAngles( "tag_deathfx" );		
	playFx( level._chopper_fx["explode"]["air_death"]["littlebird"], self getTagOrigin( "tag_deathfx" ), anglesToForward( deathAngles ), anglesToUp( deathAngles ) );
...
lbSpin( speed )
{
	...
	playfxontag( level._chopper_fx["explode"]["medium"], self, "tail_rotor_jnt" );
 	self thread trail_fx( level._chopper_fx["smoke"]["trail"], "tail_rotor_jnt", "stop tail smoke" );
