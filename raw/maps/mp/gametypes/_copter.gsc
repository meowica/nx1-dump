init()
{
	level._coptermodel = "vehicle_cobra_helicopter_fly";
	precacheModel(level._coptermodel);
	
	level._copter_maxaccel = 200;
	level._copter_maxvel = 700;
	level._copter_rotspeed = 90; // degrees per second
	level._copter_accellookahead = 2; // seconds
	
	level._copterCenterOffset = (0,0,72);
	level._copterTargetOffset = (0,0,45);
	
	level._copterexplosion = loadfx("explosions/default_explosion");
	level._copterfinalexplosion = loadfx("explosions/large_vehicle_explosion");
}
...
createCopter(location, team, damagetrig)
{
	...
	copter setModel(level._coptermodel);
	...
	while(1)
	{
		...
		if (flying)
		{
			...
			if (isdefined(best)) {
				...
				attackpos = best.origin + level._copterTargetOffset;
				...
				self.desiredDirEntityOffset = level._copterTargetOffset;
		...
		else
		{
			...
			self.desiredDir = vectornormalize(descendingEnt.targetEnt.origin - (goToPos - level._copterCenterOffset));
			self.desiredDirEntity = descendingEnt.targetEnt;
			self.desiredDirEntityOffset = (0,0,0);
			
			if (goToPos != self.origin) {
				self setCopterDest(goToPos - level._copterCenterOffset, true, goDirectly);
				wait(.3);
			}
			else
				wait(.3);
...
determineBestPos(targets, descendEnt, startorigin)
{
	...
	{
		enemypos = targets[i].origin + level._copterTargetOffset;
...
copterShoot()
{
	...
	while(1)
	{
		if (isdefined(self.desiredDirEntity) && isdefined(self.desiredDirEntity.origin)) {
			mypos = self.origin + level._copterCenterOffset;
...
myMagicBullet(pos, dir)
{
	...
	if (isdefined(trace["entity"]) && isplayer(trace["entity"]) && isalive(trace["entity"])) {
		// hurt entity shot at
		trace["entity"] thread [[level._callbackPlayerDamage]](
...
copterMove()
{
	...
	while(1)
	{
		...
		//if (!doneMoving)
		{
			...
			idealAccel = vecscale(thisDest - (self.origin + vecscale(self.vel, level._copter_accellookahead)), interval);
			vlen = veclength(idealAccel);
			if (vlen > level._copter_maxaccel) {
				idealAccel = vecscale(idealAccel, level._copter_maxaccel / vlen);
			}
			
			self.vel = self.vel + idealAccel;
			vlen = veclength(self.vel);
			if (vlen > level._copter_maxvel) {
				self.vel = vecscale(self.vel, level._copter_maxvel / vlen);
			}
			...
			if (isdefined(self.desiredDirEntity) && isdefined(self.desiredDirEntity.origin)) {
				self.destDir = vectornormalize((self.desiredDirEntity.origin + self.desiredDirEntityOffset) - (self.origin + level._copterCenterOffset));
			}
				...
				tiltamnt = speed / level._copter_maxvel;
		...
		thisRotSpeed = level._copter_rotspeed;
		//if (movelen < 256)
...
copterDie()
{
	...
	playfx (level._copterfinalexplosion, self.origin);
...
copterExplodeFX()
{
	...
	while(1)
	{
		playfx (level._copterexplosion, self.origin);
