...
main()
{
	...
	registerScoreLimitDvar( level._gameType, 101, 0, 10000 );
	registerRoundLimitDvar( level._gameType, 1, 0, 30 );
	registerWinLimitDvar( level._gameType, 1, 0, 10 );
	registerRoundSwitchDvar( level._gameType, 0, 0, 30 );
...
onDeadEvent( team )
{
	if ( ( isDefined( level._nukeIncoming ) && level._nukeIncoming ) || ( isDefined( level._nukeDetonated ) && level._nukeDetonated ) )
		return;
...
overtimeThread( time )
{
	...
	level._endGameTime = 120;
	
	time = level._endGameTime;
	...
	setGameEndTime( int( getTime() + ( level._endGameTime * 1000 ) ) );
...
{
	if ( !isDefined( level._inOvertime ) || !level._inOvertime )
		return;
