...
init()
{
	...
	*/
	...
	level._lootMins["epic"] = 90;
	level._lootMins["rare"] = 70;
	//level.lootMins["common"] = 95;
	
	level._lootIndices["epic"] = 2;
	level._lootIndices["rare"] = 1;
	level._lootIndices["common"] = 0;
	
	level._lootBaseChance = 0.05;
	level._lootIdealTime = (15*60);
...
updateLootDvars()
{
	if ( getDvar( "scr_loot_epicMin" ) == "" )
		setDvar( "scr_loot_epicMin", level._lootMins["epic"] );

	if ( getDvar( "scr_loot_rareMin" ) == "" )
		setDvar( "scr_loot_rareMin", level._lootMins["rare"] );

	//if ( getDvar( "scr_loot_commonMin" ) == "" )
	//	setDvar( "scr_loot_commonMin", level.lootMins["common"] );

	if ( getDvar( "scr_loot_baseChance" ) == "" )
		setDvar( "scr_loot_baseChance", level._lootBaseChance );

	if ( getDvar( "scr_loot_idealTime" ) == "" )
		setDvar( "scr_loot_idealTime", level._lootIdealTime );
	...
	for ( ;; )
	{
		...
		level._lootMins["epic"] = getDvarFloat( "scr_loot_epicMin" );
		level._lootMins["rare"] = getDvarFloat( "scr_loot_rareMin" );
		//level.lootMins["common"] = getDvarFloat( "scr_loot_commonMin" );
		
		level._lootBaseChance = getDvarFloat( "scr_loot_baseChance" );
		level._lootIdealTime = getDvarFloat( "scr_loot_idealTime" );
...
gotLoot()
{
	...
	baseChance = level._lootBaseChance;
	idealLootTime = level._lootIdealTime;
...
{
	if ( lootRoll >= level._lootMins["epic"] )
		return "epic";
	else if ( lootRoll >= level._lootMins["rare"] )
		return "rare";
	else
		return "common";
}
...
giveLoot( victim )
{
	...
	tierInt = level._lootIndices[lootTier];
...
{
	self thread showLootNotify( lootTier );
	self thread [[level._onXPEvent]]( lootTier );
	self CreateLootMail( lootName );
...
{
	killStreakNames = getArrayKeys( level._killstreakFuncs ); 
	killStreaks = [];
